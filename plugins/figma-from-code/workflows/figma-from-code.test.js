...
// Verify each schema parses as valid JSON-like structure. Schemas may reference
// shared sub-schema constants (e.g. const TIERS_ITEMS = {...}), so eval all
// schema-shaped const declarations cumulatively in source order.
let schemaScope = '';
const allSchemaConsts = [...src.matchAll(/const (\w+(?:_SCHEMA|_ITEMS))\s*=\s*(\{[\s\S]*?\n\})/g)];
for (const [, name, body] of allSchemaConsts) {
  try {
    eval(schemaScope + '(' + body + ')');
    schemaScope += 'const ' + name + ' = ' + body + ';\n';
    if (name.endsWith('_SCHEMA')) pass(name + ' parses as valid object');
  } catch (e) {
