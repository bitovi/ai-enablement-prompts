...
### Counting items for `.map()` / iteration patterns

When the source code renders children via iteration (`.map()`, `.forEach()`, `for...of`), `usageCount` depends on the data passed at runtime, not the static JSX. Determine the actual count using this priority order:

1. **App screenshot + text.json (highest priority):** If `text.json` exists, count the distinct repeated items visible in the text data. If `app.png` exists, visually confirm the count. The app screenshot is ground truth.
2. **Source code data:** Read the consuming component to find the actual data array passed as props (e.g., `menuItems` in `App.tsx` passed to `<MenuList items={menuItems} />`). Trace the import chain, count the array elements, and use the actual labels.
3. **Storybook stories (last resort):** Story files often use mock data with different items, different counts, or placeholder labels. Never prefer Storybook data over the app screenshot or the source code's actual prop data.

Set `usageCount` to the count from the highest-priority source available. In the `usages` array, use the actual labels from that same source.
...
### When no app screenshot or text.json exists (`no_app_reference` path)

If this component has no `app.png` and no `text.json` (it was skipped during precapture), derive text content and item counts from the source code:

1. **Find the consuming component:** Grep for imports of the component name to find where it's used. Read the JSX where it's rendered to find the actual props being passed.
2. **Trace prop data to its definition:** If a prop is a variable (e.g., `items={menuItems}`), find where it's defined and count its elements. Use the actual labels from the definition.
3. **Storybook as last resort only:** Read Storybook stories only if the consuming component cannot be found. When using story data, note it in `textContent` as `"source": "storybook"` so downstream steps can flag it.
...
2. **Standalone call (no orchestrator)** — before falling back to source-only analysis, determine whether a dev server is running:
   - The canonical URL is `config.devServerUrl` from `state.json`. Use that if available. As a detection heuristic only (when `state.json` is absent), check the project's `package.json` `scripts.dev` and probe the likely port (Vite default: 5173, Next.js/CRA default: 3000).
...
3. **Auto/non-interactive mode** — if you cannot ask the user (e.g., running fully autonomously inside a batch), and probing finds no dev server, still attempt to start one using `{devServerStart}` if the project clearly supports it (e.g., with a port check loop). If starting the server isn't safe or appropriate, proceed without 1g but explicitly flag `"liveInspection": "skipped_no_dev_server"` in the result so the validator can re-check later.
