...
**Result file schema:** see the canonical definition in `plugins/figma-from-code/skills/figma-from-code/7-build-component/7b-review-fix-component/SKILL.md` § "Result File Schema". Write every field defined there. Key points for this step:
...
- `figmaScreenshot` and `screenshotNodeId` — copy from the `-built.json` handoff (or from the last fix-loop screenshot capture if it was updated during the fix loop)
