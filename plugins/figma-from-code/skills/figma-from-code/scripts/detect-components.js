...
const { LIBRARY_PATTERNS } = require('./library-filter');

async function discoverOnPage(page, { minSize = 4, libraryPatterns = null } = {}) {
  const libraryPatternSources = (libraryPatterns || LIBRARY_PATTERNS).map(p => p.source);
  return await page.evaluate(({ minSize, libraryPatternSources }) => {

    const libPatterns = libraryPatternSources.map(s => new RegExp(s));
    const isLibName = (name) => libPatterns.some(p => p.test(name));
    ...
    function isComponentFiber(node) {
      return typeof node.type === 'function' || (typeof node.type === 'object' && node.type !== null);
    }

    function fiberName(node) {
      let t = node.type;
      if (!t) return null;
      if (typeof t === 'object' && t.$$typeof === Symbol.for('react.memo') && t.type) t = t.type;
          ...
          while (node) {
            if (isComponentFiber(node)) {
              ...
              if (name) {
                ...
                let attrName = name;
                if (isLibName(name)) {
                  let candidate = node.return;
                  let hops = 0;
                  while (candidate && hops < 10) {
                    if (isComponentFiber(candidate)) {
                      const candidateName = fiberName(candidate);
                      if (candidateName && !isLibName(candidateName)) {
                        attrNode = candidate;
                ...
                if (seenFibers.has(attrNode)) break;
                seenFibers.add(attrNode);
                const props = extractVisualProps(attrNode.memoizedProps);
                let parentFiber = attrNode.return;
                let fiberParent = null;
                while (parentFiber) {
                  if (isComponentFiber(parentFiber)) {
                    const parentName = fiberName(parentFiber);
                    if (parentName && parentName !== attrName && !isLibName(parentName)) {
                ...
                components.push({ name: attrName, el, props, fiberParent });
                break;
              }
            }
            node = node.return;
    ...
    function positionalSegments(el, stopAt) {
      const segments = [];
      let current = el;
      while (current && current !== stopAt && current !== document.body && segments.length < 5) {
    ...
    function findStableAnchor(el) {
      ...
      while (cur && cur !== document.body) {
        if (cur.dataset?.testid) return { el: cur, sel: `[data-testid="${cur.dataset.testid}"]` };
        if (cur.id && /^[a-zA-Z]/.test(cur.id) && isUniqueSelector(`#${CSS.escape(cur.id)}`)) {
          return { el: cur, sel: `#${CSS.escape(cur.id)}` };
        }
        const ariaLabel = cur.getAttribute('aria-label');
        if (ariaLabel) {
          const sel = `${cur.tagName.toLowerCase()}[aria-label="${ariaLabel}"]`;
          if (isUniqueSelector(sel)) return { el: cur, sel };
        }
        const role = cur.getAttribute('role');
        if (role && /^(dialog|alertdialog|menu|listbox|tooltip)$/.test(role)) {
          return { el: cur, sel: `[role="${role}"]` };
        }
        if (/^(HEADER|MAIN|NAV|ASIDE|FOOTER|DIALOG)$/.test(cur.tagName)) {
          const sel = cur.tagName.toLowerCase();
          if (isUniqueSelector(sel)) return { el: cur, sel };
        }
    ...
    function generateSelectorCandidates(el) {
      const candidates = [];
      const push = (sel) => { if (sel && !candidates.includes(sel)) candidates.push(sel); };
      const tag = el.tagName.toLowerCase();

      if (el.dataset?.testid) push(`[data-testid="${el.dataset.testid}"]`);

      if (el.id && /^[a-zA-Z]/.test(el.id) && isUniqueSelector(`#${CSS.escape(el.id)}`)) {
      ...
      if (ariaLabel) {
        ...
        if (isUniqueSelector(sel)) push(sel);
      }
      ...
      const anchor = findStableAnchor(el);
      if (anchor) {
        const relative = positionalSegments(el, anchor.el);
        if (relative.length > 0 && relative.length <= 4) {
          const sel = `${anchor.sel} ${relative.join(' > ')}`;
          if (isUniqueSelector(sel)) push(sel);
        }
      }
      ...
      if (/^(input|textarea|select)$/.test(tag)) {
        ...
        if (inputType) {
          const sel = `${tag}[type="${inputType}"]`;
          if (isUniqueSelector(sel)) push(sel);
        }
      }

      push(positionalSegments(el, document.body).join(' > '));
      return candidates;
    }
    ...
    for (const { name, el } of rawComponents) {
      ...
      let isVisible = box.width > 0 && box.height > 0 &&
      ...
      const candidates = generateSelectorCandidates(effectiveEl);
      if (effectiveEl !== el) {
        const parentCandidates = generateSelectorCandidates(el);
        for (const pc of parentCandidates) {
          if (!candidates.includes(pc)) candidates.push(pc);
        }
      }
      ...
      deduped.get(name).push({
        ...
        candidates,
        box: { x: Math.round(effectiveBox.x), y: Math.round(effectiveBox.y),
               width: Math.round(effectiveBox.width), height: Math.round(effectiveBox.height) },
        visible: isVisible,
